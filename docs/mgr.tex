...
\polishabstract {Interpretacja i kompilacja języków programowania stanowi kluczową część informatyki - żaden język nie byłby użyteczny, gdyby nie istniały narzędzia, które potrafiłyby go zrozumieć. Proces sam w sobie jest bardzo skomplikowany, ponieważ obejmuje on kilka kroków, takich jak analiza leksykalna czy też parsowanie. Jak się okazuje, jest on również ściśle powiązany z matematyką, gdyż każdy język programowania może być opisany jako zbiór formalnych reguł, które składają się na semantykę języka. Jeżeli semantyka jest dobrze zdefiniowana, interpretowanie języka sprowadza się do odzwierciedlenia reguł semantycznych w kodzie źródłowym. Inna ważna kwestia odnosi się do tego, w jaki sposób interpreter bądź kompilator może być testowany. Niniejsza praca skupia się na teoretycznych aspektach \acrshort{cil} - pośredniego języka programowania stworzonego przez \texttt{Microsoft}. Wprowadza ona do samego języka, studiuje jego dokumentację i bada jego semantykę. Praktyczna część pracy zawiera interpreter języka \acrshort{cil} oraz dostarcza metodologię, która została użyta podczas testowania wypracowanego rozwiązania.}

\englishabstract{

Interpretation and compilation of programming languages is a crucial part of information technology - no language would be usable if there were no tools that could understand it. The process itself is very complex as it involves several steps such as lexical analysis or parsing. As it turns out, it is also strictly related to mathematics as each programming language can be described as a set of formal rules that compose the semantics of the language. If the semantics is well-defined, interpreting the language comes down to reflecting semantic rules in source code. Another important issue is related to how an interpreter or a compiler can be tested. This thesis focuses on the theoretical aspects of the \acrshort{cil} - an intermediate programming language created by \texttt{Microsoft}. It provides an introduction to the language itself, studies its documentation and investigates its semantics. The practical part of the thesis includes an interpreter of the \acrshort{cil} and provides a methodology that was used while testing the developed solution.}
